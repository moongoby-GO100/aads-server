...
-- Preserve active-grant immutability while permitting this migration's
-- one-time, derivable lineage backfill.  Once populated, these columns remain
-- immutable just like every other scope field.
CREATE OR REPLACE FUNCTION aads_guard_active_grant() RETURNS TRIGGER
LANGUAGE plpgsql AS $$
DECLARE
    mutable_columns TEXT[] := ARRAY[
        'status','used_executions','revoked_by','revoked_at','revoke_reason',
        'revocation_epoch','last_used_at'
    ];
    lineage_columns TEXT[] := ARRAY[
        'logical_grant_id','root_grant_id','delegation_path',
        'parent_grant_version','parent_scope_hash','ancestor_revocation_epoch'
    ];
    lineage_backfill BOOLEAN;
BEGIN
    lineage_backfill :=
        OLD.logical_grant_id IS NULL
        AND OLD.root_grant_id IS NULL
        AND cardinality(OLD.delegation_path) = 0
        AND NEW.logical_grant_id = OLD.id
        AND NEW.root_grant_id IS NOT NULL
        AND cardinality(NEW.delegation_path) > 0
        AND NEW.delegation_path[cardinality(NEW.delegation_path)] = OLD.id
        AND (to_jsonb(NEW) - mutable_columns - lineage_columns)
            IS NOT DISTINCT FROM
            (to_jsonb(OLD) - mutable_columns - lineage_columns);
    IF OLD.status = 'active'
       AND (to_jsonb(NEW) - mutable_columns) IS DISTINCT FROM
           (to_jsonb(OLD) - mutable_columns)
       AND NOT lineage_backfill THEN
...
UPDATE goal_auto_approval_grants grant_row
   SET logical_grant_id=COALESCE(grant_row.logical_grant_id,grant_row.id),
       root_grant_id=lineage.root_id,
       delegation_path=lineage.path
