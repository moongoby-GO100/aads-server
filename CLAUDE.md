...
3. **코드가 막는 것과 문서 규칙을 구분해 적는다.** 이 서버에서 명령을 실행하는
   것은 거의 전부 에이전트다 — `docker compose build` 를 친 것도, 샤드를 늘린
   것도 사람이 아니라 세션이다. 그리고 에이전트는 **자기가 쓴 규칙도 어긴다.**
   2026-09-14 "릴리스 중 병행 빌드 금지" 를 AGENTS.md 에 써넣은 세션이 같은 날
   그 규칙을 어겨 배포 #414 를 죽였다.

   그러니 fix 에 "규칙으로만 기록" 이라고 적힌 항목은 **"또 일어난다"** 로
   읽어야 한다. 두 번 이상 어긴 규칙은 코드 차단으로 옮긴다 — 중복·병행 빌드는
   그렇게 deploy.sh 로 옮겼다(a679652).
...
- **pre-commit hook**: ①API 키 탐지 ②구문 검사 ③ruff 정적 분석 ④Docker import 검증 ⑤단위 테스트 ⑥중복 재적용 차단(`scripts/dup_guard.py`) — 모두 통과해야 커밋 가능.
  - ③은 **지금 실제로 돈다** — `ruff 0.16.7` 이 `/usr/local/bin/ruff` 에 설치돼 있다(2026-09-17 실측). 검사 범위는 스테이징된 파일의 `F821`(정의 안 된 이름)·`F811`(중복 정의) 두 규칙뿐이고, 나머지 8천여 건(대부분 `I001` import 정렬)은 게이트가 보지 않는다.
  - 2026-09-15 에는 ruff 가 없어 이 단계가 한 번도 돌지 않았고, 그 사이 F811 이 잡히지 않아 같은 패치가 세 번 커밋됐다. **이 문서가 "돈다"고 적고 있는 동안 실제로는 건너뛰고 있었던 것**이 사고의 원인이었으므로, 설치 상태가 바뀌면 이 줄을 반드시 같이 고쳐라. 확인은 `command -v ruff` 한 줄이다.
