...
#
# ── 2026-09-16 실측: 이 모드를 켜도 효과가 작다 ─────────────────────────────
#
# 도구 142개 스키마는 **76,245자**지만 모델 입력은 그만큼 늘지 않는다. CLI 가
# 전체 스키마를 프롬프트에 그대로 싣지 않기 때문이다. 옵션을 빼며 잰 차분:
#
#   MCP 없음          20,331 토큰
#   MCP 전량 142개    22,208 토큰   → 도구 전체 비용 **1,768 토큰**
#   MCP 상주  11개    20,481 토큰   → 상주 모드 절감 **1,727 토큰 (9.7%)**
#
# 즉 도구 하나당 약 13 토큰이고, 이는 이름·요약 수준이다. 처음에 글자 수를
# 3으로 나눠 "25,000 토큰" 으로 추정했는데 **14배 과대**였다. 그 추정 때문에
# 우선순위를 거꾸로 잡았다 — 실제 덩어리는 시스템 프롬프트(26,677토큰)였다.
#
# 그래서 이 모드는 기본 꺼짐이다. 9.7% 를 얻자고 "도구를 못 찾는다" 는 실패를
# 감수할 이유가 없다. 도구가 300개 이상으로 늘면 그때 다시 재고 켜라.
