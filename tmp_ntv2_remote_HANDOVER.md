...
## 2026-05-05 KST 운영 조치 메모

- V1 pick 관리자 상품수정 이미지 엑박 보완 적용.
- 대상: `/home/newpigup3/application/controllers/Product.php`, `/home/newpigup3/views/root/product_mng_prodregister.php`, `/home/newpigup3/views/root/product_mng.php`.
- 원인: `https://www.newtalk.kr/data/files/goods/5/{hash}` 공개 URL이 Cloudflare에서 403을 반환하지만 실제 파일은 `/home/danharoo/www/data/files/goods/5/`에 존재.
- 조치: 화면 표시용 이미지 `src`를 로그인 세션 기반 `/Product/product_image_file`로 전환하고, 상품목록 대표 이미지 fallback을 `/Product/goodscode_image_file`로 전환. DB 값과 삭제 Ajax용 원본 URL은 유지.
- 백업: 각 파일에 `.bak_20260505_1045_image_proxy` 생성.
- 검증: PHP lint 3개 파일 통과, 비로그인 요청은 로그인 리다이렉트 확인. 실제 화면 렌더링은 로그인 세션 브라우저에서 확인 필요.
...
## 2026-05-05 상품/회원 활성화 UI 기획 v1.9 기록

- CEO 지시: V1의 실제 서비스와 V2에 현재 적용된 기능·디자인을 함께 참고해 기획서 보고.
- 산출물:
  - Next public: `/srv/newtalk-v2/frontend/public/NT-PRODUCT-REGISTRATION-UI-PLAN-v1.9-20260505.html`
  - Laravel public: `/srv/newtalk-v2/src/public/NT-PRODUCT-REGISTRATION-UI-PLAN-v1.9-20260505.html`
  - 브라우저 URL: `https://v2.newtalk.kr/NT-PRODUCT-REGISTRATION-UI-PLAN-v1.9-20260505.html`
- 확인 근거:
  - V1 전체 상품 78,026건, V1 `mall_activated=Y` 57,659건.
  - V2 `products` 77,349건, `users` 79,729건, `wholesale_profiles` 2,179건, `retail_profiles` 0건, `orders` 6건.
  - V2 프론트 경로: `/retail/feed`, `/retail/product/[id]`, `/wholesale/dashboard`, `/wholesale/shooting` 기획 반영.
  - 기존 V2 디자인 자산: `frontend/public`, `src/app/(retail)`, `src/components/feed`, `src/components/product`, `src/components/retail`, `src/components/dropship`.
- 반영 내용:
  - 관리자 V2화는 후순위로 두고 V1 관리자를 운영 기준선으로 유지.
  - 소매회원은 상품 발견, 이미지/상세 패키지, 외부몰 자료, 드롭십 주문 전환을 최우선 화면으로 정의.
  - 도매회원은 상품등록 강제 대신 촬영 요청, 이미지 제출, 뉴톡 대행등록, 소매 반응 지표 중심 콘솔로 정의.
  - 상품등록은 관리자 입력폼이 아니라 소매/도매 활성화 과정에서 필요한 상품 완성 워크플로우로 재정의.
- 검증:
  - 외부 URL `HTTP 200` 확인.
  - HTML 핵심 섹션 `V1 서비스 구조 요약`, `V2 현재 적용 기능과 디자인`, `소매회원 활성화 UI 디자인안`, `도매회원 활성화 UI 디자인안`, `관리자 정책` 확인.
  - Chromium 렌더 캡처: `reports/2026-05-05/assets/report-v1.9-render-check.png`.
