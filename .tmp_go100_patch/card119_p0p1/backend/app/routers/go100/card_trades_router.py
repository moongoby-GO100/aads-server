...
_STAGE1_COLUMNS: dict[str, list[dict]] = {
    'limitup_chase': [
        ...
        {"key": "first_watch20_at", "label": "+20 최초", "type": "time"},
        {"key": "first_buy27_at", "label": "+27 최초", "type": "time"},
        {"key": "first_limitup_at", "label": "상한가 최초", "type": "time"},
        {"key": "limitup_unlock_count", "label": "풀림횟수", "type": "number"},
        {"key": "last_limitup_at", "label": "상한가 최종", "type": "time"},
...
def _build_card119_strategy_definition(
    *,
    ...
    return {
        ...
        "discovery": {
            "status": "implemented",
            "filter": "preopen expected_change_rate >= 20.0 OR intraday change_pct >= 20.0 AND trading_value_krw >= 100,000,000",
            ...
            "note": "+20%는 watch 발굴, +27%는 BUY 하드게이트입니다. 공통 v4_scalping_universe 50종목을 쓰지 않습니다.",
        },
        "selection": {
            "status": "implemented",
        ...
        "entry": {
            ...
            "fully_locked_policy": "continue risk/account/broker gates instead of pre-card blocking",
...
async def _stage1_card119_reach_timing(
    db: AsyncSession,
    *,
    stock_codes: list[str],
    trade_date: Any,
) -> dict[str, dict[str, Any]]:
    """Return #119 intraday threshold reach times from observable minute bars."""
    clean_codes = sorted({str(code).strip() for code in stock_codes if str(code).strip()})
    if not clean_codes:
        return {}
    try:
        timing_r = await db.execute(
            text("""
                WITH prev_close AS (
                    SELECT c.stock_code,
                           COALESCE((
                               SELECT od.close
                               FROM ohlcv_daily od
                               WHERE od.stock_code = c.stock_code
                                 AND od.date < to_char(CAST(:trade_date AS date), 'YYYYMMDD')
                               ORDER BY od.date DESC
                               LIMIT 1
                           ), 0) AS prev_close
                    FROM unnest(CAST(:stock_codes AS text[])) AS c(stock_code)
                ),
                bars AS (
                    SELECT
                        m.stock_code,
                        (m.trade_date::timestamp + m.trade_time::interval) AS bar_at,
                        GREATEST(
                            COALESCE(m.high_price, 0),
                            COALESCE(m.close_price, 0)
                        )::numeric AS observed_price,
                        NULLIF(pc.prev_close, 0)::numeric AS prev_close
                    FROM v4_ohlcv_minute m
                    JOIN prev_close pc ON pc.stock_code = m.stock_code
                    WHERE m.stock_code = ANY(:stock_codes)
                      AND m.trade_date = CAST(:trade_date AS date)
                ),
                flags AS (
                    SELECT
                        stock_code,
                        ...
                        CASE
                            WHEN prev_close > 0
                            THEN ((observed_price / prev_close) - 1) * 100
                            ELSE NULL
                        END AS change_pct
                    FROM bars
                ),
                state AS (
                    SELECT
                        ...
                        bar_at,
                        change_pct,
                        (change_pct >= 20.0) AS watch20,
                        (change_pct >= 27.0) AS buy27,
                        (change_pct >= 29.8) AS limitup_locked
                    FROM flags
                ...
                lagged AS (
                    SELECT
                        *,
                        LAG(limitup_locked, 1, false) OVER (
                            PARTITION BY stock_code ORDER BY bar_at
                        ) AS prev_limitup_locked
                    FROM state
                )
                SELECT
                    stock_code,
                    MIN(bar_at) FILTER (WHERE watch20) AS first_watch20_at,
                    MIN(bar_at) FILTER (WHERE buy27) AS first_buy27_at,
                    MIN(bar_at) FILTER (WHERE limitup_locked) AS first_limitup_at,
                    MAX(bar_at) FILTER (WHERE limitup_locked) AS last_limitup_at,
                    COUNT(*) FILTER (WHERE prev_limitup_locked AND NOT limitup_locked)::integer AS limitup_unlock_count,
                    MAX(change_pct) AS max_intraday_change_pct,
                    COUNT(*) FILTER (WHERE limitup_locked)::integer AS limitup_locked_bar_count
                FROM lagged
                GROUP BY stock_code
            """),
            {"stock_codes": clean_codes, "trade_date": trade_date},
        )
        result: dict[str, dict[str, Any]] = {}
        for row in timing_r.fetchall():
            result[str(row.stock_code)] = {
                "first_watch20_at": _ts(row.first_watch20_at),
                "first_buy27_at": _ts(row.first_buy27_at),
                "first_limitup_at": _ts(row.first_limitup_at),
                "last_limitup_at": _ts(row.last_limitup_at),
                "limitup_unlock_count": int(row.limitup_unlock_count or 0),
                "max_intraday_change_pct": _safe_float(row.max_intraday_change_pct),
                "limitup_locked_bar_count": int(row.limitup_locked_bar_count or 0),
                "timing_source": "v4_ohlcv_minute",
            }
        return result
    except Exception as exc:
        logger.warning("workbench #119 reach timing query failed: %s", exc)
        return {}
...
async def _build_stage1_card119_independent_stage(
    ...
    *,
    ...
    reach_timing = await _stage1_card119_reach_timing(
        db,
    ...
    for raw_row in candidate_rows:
        ...
        rendered.update(reach_timing.get(str(code), {}))
        visible_rows.append(rendered)
        status_counts[candidate_status] = status_counts.get(candidate_status, 0) + 1
    ...
    return {
        ...
        "summary": {
            ...
            "reach_timing_source": "v4_ohlcv_minute",
            "watch20_reached_count": sum(1 for row in visible_rows if row.get("first_watch20_at")),
            "buy27_reached_count": sum(1 for row in visible_rows if row.get("first_buy27_at")),
            "limitup_reached_count": sum(1 for row in visible_rows if row.get("first_limitup_at")),
            "limitup_unlock_total": sum(int(row.get("limitup_unlock_count") or 0) for row in visible_rows),
            "by_status": status_counts,
        },
    }
