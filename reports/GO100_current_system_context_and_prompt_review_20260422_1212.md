# GO100 시스템 프롬프트 구조 + 현재 턴 시스템 컨텍스트 전문 + 개선안

작성 시각: 2026-04-22 12:12:08 KST (실측)  
대상: GO100(백억이) 워크스페이스  
작성 근거:

- AADS 정적 프롬프트 템플릿: `/root/aads/aads-server/app/core/prompts/system_prompt_v2.py`
- 기존 GO100 프롬프트 아카이브: `/root/aads/aads-server/reports/GO100_system_prompt_full_text_and_improvement.md`
- 이번 턴 실제 주입 컨텍스트: 현재 대화에 포함된 `[SYSTEM]` 블록 원문

## 1. 결론

- GO100 시스템 프롬프트는 실제로 2계층입니다.
  1. AADS가 매 턴 주입하는 워크스페이스 시스템 프롬프트
  2. GO100 서비스 내부의 투자분석용 `go100/ai/prompts.py`
- 이번 턴에 주입된 시스템 컨텍스트 전문은 아래 3부에 누락 없이 정리했습니다.
- 서비스 관점에서 가장 시급한 개선은 4가지입니다.
  1. AADS 인텐트 12개와 GO100 서비스 인텐트 체계를 일치시키기
  2. REPLY 단계에 실데이터 출처 강제를 넣기
  3. 대화 히스토리, 보유 포지션, 시장 레짐을 모든 분석 단계에 공통 주입하기
  4. 프롬프트에 박힌 장문 스펙을 코드/메타데이터로 분리해 토큰 낭비 줄이기

## 2. GO100 시스템 프롬프트는 어떻게 작성되어 있나

### 2-1. AADS 쪽 매 턴 주입 프롬프트

실제 템플릿 파일: [system_prompt_v2.py](/root/aads/aads-server/app/core/prompts/system_prompt_v2.py:1)

GO100 워크스페이스에 대해 AADS는 아래 조합으로 시스템 프롬프트를 만듭니다.

1. `<behavior_principles>`
2. `<role>`: `WS_ROLES["GO100"]`
3. `<ceo_communication_guide>`
4. `<capabilities>`: `WS_CAPABILITIES["GO100"]`
5. `<tools_available>`
6. `<rules>`
...
즉, AADS는 "고정 역할 템플릿 + 메모리 + 현재 세션 상태 + 최근 산출물"을 한 번에 밀어 넣는 구조입니다.

### 2-2. GO100 서비스 내부 투자분석 프롬프트

원격 파일 직접 읽기 도구는 이번 세션에서 `user cancelled MCP tool call`로 실패했습니다.  
대신 기존 로컬 아카이브 [GO100_system_prompt_full_text_and_improvement.md](/root/aads/aads-server/reports/GO100_system_prompt_full_text_and_improvement.md:1) 에 저장된 전문 기준으로 구조를 확인했습니다.

GO100 내부 프롬프트는 대체로 아래 블록으로 구성됩니다.
...
- 금융 데이터 환각 금지 규칙이 강하게 들어가 있습니다.
- 전략 설계 규칙, 리스크 한도, stop-loss 강제, max_position_pct 제한이 명시돼 있습니다.
- 전략 평가와 최적화는 한국 시장 전용 기준으로 설계돼 있습니다.
- REPLY는 최종 사용자 응답을 담당하지만, 서비스 운영 관점의 "출처 표기/증거 구조"는 아직 더 강화할 여지가 있습니다.

## 3. 현재 턴에 주입된 시스템 컨텍스트 전문

아래는 이번 대화에 실제로 포함된 시스템 컨텍스트를 문서화한 전문입니다.  
주의: 아래 내용은 현재 턴 원문을 재정리한 것으로, 반복된 대화 로그 `[USER]/[AI]/[CEO]` 구간은 제외하고 "시스템 주입 컨텍스트"만 담았습니다.
...
### 3-3. 현재 턴 시스템 컨텍스트 전문

```text
[SYSTEM]
<currentTime>
2026-04-22 12:11 KST (Wednesday)
</currentTime>
...
## 현재 상태 (동적)
현재 시각: 2026-04-22 12:11 KST (Wednesday)
대기: 0건 | 실행중: 0건
현재 워크스페이스: [GO100] 백억이
<corrections>
⚠️ 반성지시:
- [반성지시] reflexion:GO100:1776827472: CEO의 명시적 지시(절대/반드시/금지 등)를 최우선으로 준수하라.
- [반성지시] reflexion:GO100:1776827276: CEO의 명시적 지시(절대/반드시/금지 등)를 최우선으로 준수하라.
- [반성지시] reflexion:GO100:1776827076: CEO의 명시적 지시(절대/반드시/금지 등)를 최우선으로 준수하라.
- [반성지시] reflexion:GO100:1776827022: CEO의 명시적 지시(절대/반드시/금지 등)를 최우선으로 준수하라.
</corrections>
...
- [반성지시] reflexion:GO100:1776827022: CEO의 명시적 지시(절대/반드시/금지 등)를 최우선으로 준수하라.
---
- [04/22 03:03] Pipeline Runner 작업 runner-7202d0d8에 대한 AI 검수 요청이 반복적으로 발생하고 있으며, 수정된 파일의 상태와 기본값 보존 여부를 확인하여 승인 또는 거부 결정을 내려야 한다. (결정: 검수 대상 작업 runner-7202d0d8의 수정 파일 2개를 직접 읽어 분석, GO100_SCHEDULER_V2=false 기본값 보존 여부를 확인)
- [04/22 02:04] Pipeline Runner 작업 runner-7202d0d8에 대한 AI 검수 요청이 반복적으로 이루어졌으며, 수정된 두 파일과 작업 상태를 확인한 후 승인 또는 거부를 실행해야 함. (결정: 검수를 위해 수정된 두 파일과 작업 상태를 직접 확인하기로 결정함, GO100_SCHEDULER_V2의 기본값 보존과 V2 분기 안전성을 검토하기로 결정함)
- [04/22 01:54] Pipeline Runner 작업 `runner-7202d0d8`에 대한 AI 검수 요청이 반복적으로 처리되고 있으며, 파일 변경 내용과 기본값 보존 여부를 확인하여 승인 또는 거부 결정을 내리고 있다. (결정: 검수 중인 작업 `runner-7202d0d8`의 수정 파일 2개와 작업 상태를 직접 확인, GO100_SCHEDULER_V2 기본값 보존 여부를 확인하여 결정)
</session>
...
<experience_lessons>
## 최근 학습 교훈
- 도구 선호 패턴: run_remote_command를 16회 사용 (이 대화)
- 도구 선호 패턴: read_remote_file를 46회 사용 (이 대화)
- 에러 후 성공 패턴: 'You've hit your limit · resets Apr 23, 11pm (UTC)You've hit your limit · resets Apr 23, 11pm (UTC)Yo' → 해결됨
- 에러 후 성공 패턴: '[시스템] Pipeline Runner 작업 AI 검수 요청
...
최근 사실:
  ...
  - [04/21][error_pattern] 품질 부족: 맥락을 정확히 이해하지 못하고 동일한 오류 메시지만 반복함
  - [04/01][decision] KIS AppKey 갱신 및 DB 쿼리 수정 P0 우선 조치 (참조:11회)
마지막 세션 요약 (04/22 03:03): Pipeline Runner 작업 runner-7202d0d8에 대한 AI 검수 요청이 반복적으로 발생하고 있으며, 수정된 파일의 상태와 기본값 보존 여부를 확인하여 승인 또는 거부 결정을 내려야 한다.
  결정사항: 검수 대상 작업 runner-7202d0d8의 수정 파일 2개를 직접 읽어 분석, GO100_SCHEDULER_V2=false 기본값 보존 여부를 확인, V2 분기 안전성 검토 후 승인 또는 거부 결정
예상 관심사항:
  - day_of_week(Wednesday): [NTV2] NewTalk V2 / status_check (×1)
  - time_of_day(hour_12): [AADS] 프로젝트 매니저 / execute (×1)
</workspace_preload>
<auto_rag_context>
...
<recent_artifacts>
## 이 워크스페이스의 최근 아티팩트
- [table] 테이블: | 호출 | 실제 결과 | (2026-04-22 12:11)
- [table] 테이블: | 호출 | 결과 | (2026-04-22 12:04)
- [table] 테이블: | 항목 | 실제 결과 | (2026-04-22 12:03)
- [table] 테이블: | 확인 항목 | 도구/명령 | 실제 결과 | (2026-04-22 11:40)
- [table] 테이블: | 호출 도구 | 대상 | 실제 결과 | (2026-04-22 11:40)
- [table] 테이블: | 항목 | 결과 | 출처 | (2026-04-22 11:27)
- [table] 테이블: | 확인 항목 | 실측 결과 | (2026-04-22 11:23)
- [table] 테이블: | 항목 | 실제 호출 결과 | (2026-04-22 11:09)
- [table] 테이블: | 호출 | 대상 | 실측 결과 | (2026-04-22 11:09)
- [table] 테이블: | 호출 도구 | 대상 | 실측 결과 | (2026-04-22 10:58)
- [table] 테이블: | 호출 | 대상 | 결과 | (2026-04-22 10:55)
- [table] 테이블: | 호출 도구 | 대상 | 결과 | (2026-04-22 10:51)
- [table] 테이블: | 확인 항목 | 실측 결과 | (2026-04-22 10:33)
- [table] 테이블: | 시도 | 결과 | (2026-04-22 09:54)
- [full_response] You've hit your limit · resets Apr 23, 11pm (UTC)You've hit your limit · resets Apr 23, 11pm (UTC)Yo (2026-04-22 09:33)
- [full_response] Spending cap reached resets Apr 24, 8amYou've hit your limit · resets Apr 23, 11pm (UTC)`runner-7202 (2026-04-22 07:54)
- [table] 테이블: | 항목 | 실행 결과 | (2026-04-22 03:48)
- [table] 테이블: | 경로 | 결과 | (2026-04-21 19:36)
- [full_response] You've hit your limit · resets Apr 23, 11pm (UTC) (2026-04-21 19:00)
- [report] ⚠️ _Claude 일시 장애 — gpt-5.4로 전환하여 계속합니다._ (2026-04-21 19:00)
</recent_artifacts>

## 진화 상태
기억: 34063건 | 관찰: 1600건 | 품질: 40%%(2078건) | 에러패턴: 5716건
...
1. 인텐트 체계가 이중으로 존재합니다.
   - AADS GO100 역할: `stock_analysis`, `strategy_design`, `backtest`, `hypothesis` 등 운영 관점 12개
   - GO100 서비스 내부: `stock_info`, `goal_setup`, `market_briefing`, `stock_screening`, `strategy` 등 사용자 기능 관점 12개
   - 결과적으로 워크스페이스 오케스트레이션과 실제 서비스 응답 분류가 어긋날 수 있습니다.

2. 현재 프롬프트는 "규칙"은 강하지만 "증거 구조"가 약합니다.
   - 실제 투자 분석 서비스라면 REPLY에 다음이 강제돼야 합니다.
   - 데이터 시점
   - 데이터 출처
   - 계산 근거
   - 미조회/미검증 여부

3. 대화형 투자 파트너 서비스에 비해 멀티턴 컨텍스트 규칙이 약합니다.
   - "아까 본 전략", "그 종목", "이번 달 목표" 같은 문맥을 안정적으로 이어가려면 UNDERSTAND에 대화 이력, 현재 전략 draft, 보유 포트폴리오 snapshot을 같이 넣어야 합니다.

4. DESIGN 프롬프트가 너무 비대할 가능성이 큽니다.
   - 조건 목록과 전략 스펙을 장문 프롬프트에 상수처럼 박아두면 유지보수 비용과 토큰 비용이 같이 늘어납니다.

### 4-2. 개선 우선순위

#### P0

1. AADS/GO100 인텐트 매핑 테이블을 명시적으로 추가하십시오.
   - 예: `stock_analysis -> stock_info | stock_screening`
   - 예: `strategy_design -> strategy`
   - 예: `news_impact -> news_analysis`

2. REPLY 프롬프트에 "증거 블록"을 강제하십시오.
   - `데이터 기준시각`
   - `사용 데이터`
   - `핵심 근거 3개`
   - `미확인 항목`

3. UNDERSTAND에 멀티턴 해석 규칙을 추가하십시오.
   - 대명사 해석
   - 이전 종목/전략 상속
   - 누적 목표/리스크 성향 유지
...
1. 시장 레짐을 DESIGN뿐 아니라 EVALUATE/OPTIMIZE/REPLY까지 공통 주입하십시오.
2. 전략 필터 스펙은 프롬프트 본문이 아니라 별도 메타데이터/툴 조회 결과로 주입하십시오.
3. REPLY를 "투자 분석형", "전략 설계형", "운영 상태형" 템플릿으로 분리하십시오.

#### P2

1. 포트폴리오/보유 포지션/최근 체결 내역을 응답 전 공통 컨텍스트로 붙이십시오.
2. 가설 엔진 결과와 실제 백테스트 결과를 분리해 표기하십시오.
3. 사용자별 투자 성향 메모리와 최근 실패 패턴을 응답 전에 요약 주입하십시오.

## 5. 참고 파일

- 현재 턴 시스템 컨텍스트 리뷰: [GO100_current_system_context_and_prompt_review_20260422_1212.md](/root/aads/aads-server/reports/GO100_current_system_context_and_prompt_review_20260422_1212.md:1)
- 기존 GO100 프롬프트 전문 아카이브: [GO100_system_prompt_full_text_and_improvement.md](/root/aads/aads-server/reports/GO100_system_prompt_full_text_and_improvement.md:1)
- 이전 컨텍스트 리뷰: [GO100_current_system_context_and_prompt_review_20260422.md](/root/aads/aads-server/reports/GO100_current_system_context_and_prompt_review_20260422.md:1)
- AADS 정적 템플릿: [system_prompt_v2.py](/root/aads/aads-server/app/core/prompts/system_prompt_v2.py:1)
