...
- 배경: CEO가 이전 완료보고의 ledger 충돌을 지적하고, "현재 AADS 기준 GO100 라우팅은 분리됐지만 211은 KIS 영향이 남아 있어 폐지가 위험하다"는 부분을 계속 확인해 최종 완료보고를 요구했다.
- 실측:
  - contabo14는 `go100`, `go100-frontend`, PostgreSQL, Redis, Nginx, cron이 모두 active였다.
  - contabo14 root crontab 활성 작업은 0건이었다.
  - legacy 211은 hostname `kis-autotrade-v4`이고 `kis-v41-api`, `kis-api`, `kis-trading`, `kis-collector` 후보 서비스는 inactive였으나 PostgreSQL, Redis, Nginx는 active였다.
  - legacy 211 root crontab에는 주석/공백 제외 189개 활성 작업이 있고, 이 중 180개가 `go100|kiwoom|kis|trading|order|balance|account|condition|ohlcv|minute|backup|unified|collect|report|token|ws` 키워드와 일치했다.
- 판정:
  - GO100 웹/API 이전은 계속 정상으로 보지만, 211 물리 폐지는 계속 차단한다.
  - 차단 사유는 KIS API systemd 본체가 아니라 211 root crontab의 예약 수집/매매/보고/백업 작업 잔존과 contabo14 crontab 미이관 상태다.
- 조치:
  - `docs/GO100-211-decommission-check-20260619.md`에 12:53 KST 재검증 수치를 추가했다.
- 다음:
  - 211 crontab을 안전/이관/폐기 그룹으로 분류한 뒤 contabo14에 승인된 항목만 duplicate guard와 함께 이관해야 한다.
  - 211 cron 비활성 리허설 1개 시장 세션을 통과하기 전에는 물리 폐지를 실행하지 않는다.

## 2026-06-19 12:54 KST - GO100 211 final decommission block evidence
- 배경: CEO가 "KIS 확인 없이 211 물리 폐지는 위험하니 즉시 확인하고 이상 없으면 바로 폐지"를 지시했다.
- 최종 실측:
  - 211 root crontab은 총 338줄, 주석/공백 제외 활성 작업 189개이며, 180개가 KIS/GO100/키움/수집/매매/보고/백업 키워드와 일치했다.
  - contabo14 root crontab은 0개다.
  - 211의 `cron`, PostgreSQL, pgbouncer, Redis, Nginx, AADS pipeline runner, AADS remote agent는 enabled 상태다.
  - 211 timer에는 `go100-scheduler@reconcile`, `go100-scheduler@paper`, `kis-autotrade-top100-211`, `go100-scheduler@report`, `go100-scheduler@live` 다음 실행이 남아 있다.
  - 211 프로세스에는 `scripts/go100/run_hypothesis_backtest.py`, AADS runner DB query, `aads_remote_agent.py`가 실행 중이고 Docker `go100-android-agent`도 up 상태다.
- 판정:
  - 물리 폐지는 실행하지 않는다. 조건부 지시의 "이상 없음"이 성립하지 않는다.
  - 폐지 전 P0는 211 cron/timer/agent/Docker workload 분류, contabo14 이관 또는 폐기 승인, 1개 시장 세션 비활성 리허설이다.
- 기록:
  - `docs/GO100-211-decommission-check-20260619.md`에 final recheck 근거를 추가했다.
- P0 분류:
  - 키워드 중복 집계 기준 token/Kiwoom auth 14건, data collection/extraction 49건, trading/paper/position/scalping 171건, reports/monitors 57건, backup/maintenance/watchdog 15건으로 확인했다.
  - 이 수치는 독립 합계가 아니라 위험 분류용이며, 다음 단계는 원문 crontab을 private backup으로 보존한 뒤 항목별 migrate/retire/keep-disabled를 결정하는 것이다.

## 2026-06-19 12:51 KST - CEO PC Agent contabo14 SSH final verification
- 배경: CEO가 이전 완료보고의 commit/push/deploy/document ledger 불일치를 지적하고, CEO PC Agent가 켜져 있는 상태에서 contabo14 자동 SSH 접근과 Codex/Claude Code 실행 가능 여부를 다시 완료 기준으로 확인하라고 지시했다.
- 실측:
  - MCP `pc_list_agents`와 `local_model_queue_status` 모두 CEO PC Agent `2e9379a1-fed`, host `abc`, Windows 10, online, heartbeat 정상으로 반환했다.
  - 활성 AADS API `127.0.0.1:8100/api/v1/pc-agent/agents`도 동일 agent online, `backend_source=local`, `online_count=1`로 반환했다.
  - CEO PC `C:\Users\rkvs3\.ssh\config`에 `Host contabo14 go100 go100-contabo`, `HostName 5.104.86.14`, `User root`, `IdentityFile ~/.ssh/id_ed25519`, `StrictHostKeyChecking accept-new` 설정이 존재했다.
  - CEO PC Agent 경유 비대화형 검증:
    - `ssh -o BatchMode=yes -o ConnectTimeout=10 contabo14 hostname` -> `contabo14`
    - `ssh -o BatchMode=yes -o ConnectTimeout=10 go100 pwd` -> `/root`
    - `ssh -o BatchMode=yes -o ConnectTimeout=10 go100-contabo whoami` -> `root`
    - `ssh -o BatchMode=yes -o ConnectTimeout=10 contabo14 codex --version` -> `codex-cli 0.141.0`
    - `ssh -o BatchMode=yes -o ConnectTimeout=10 contabo14 claude --version` -> `2.1.183 (Claude Code)`
- 배포/운영 상태:
  - AADS active container는 `aads-server`, active port는 `8100`, `/health`는 `status=ok`를 반환했다.
  - `aads-server`와 `aads-server-green` 컨테이너는 모두 healthy였다.
  - GO100 서버 `/root/kis-autotrade-v4`는 git status clean, 최근 커밋 `b40c6211 docs-go100-command-center-codex-fix-handover` 상태다.
- Git 상태:
  - AADS 로컬 main은 `origin/main`보다 7 commits ahead다.
  - `git push origin main`은 `Permission to moongoby-GO100/aads-server.git denied to deploy key`로 실패했다. 현재 환경에는 `gh` CLI와 GitHub write token 환경변수가 없어 push 대체 경로가 없다.
  - 기존 dirty runtime files `.active_container`, `.active_port`, `app/static/gallery/manifest.json`, `nginx-aads-upstream.conf`, `nginx-aads-upstream.conf.dashboard.bak`는 이번 작업 범위 밖이라 보존했다.
- 판정:
  - CEO PC 자동 SSH 접근, contabo14 접속, Codex CLI/Claude Code 실행 검증은 완료.
  - AADS 코드 push는 GitHub 쓰기 권한 부재로 미완료. write 가능한 deploy key/PAT 등록 후 `git push origin main` 재실행 필요.

## 2026-06-19 12:40 KST - CEO PC Agent contabo14 SSH and MCP fallback
- 배경: CEO가 "PC에이전트가 켜져 있는데 오프라인이라 CEO PC 내부 SSH 설정을 직접 하지 못했다"는 이전 보고를 최종 완료 기준으로 재진행하라고 지시했다.
- 실측:
  - MCP `pc_list_agents`/`pc_execute`는 빈 목록 또는 `PC_AGENT_OFFLINE`을 반환했다.
  - 활성 AADS API `127.0.0.1:8102/api/v1/pc-agent/agents`는 CEO PC Agent `2e9379a1-fed` 온라인, host `abc`, user `abc\rkvs3`, heartbeat 정상으로 반환했다.
  - CEO PC `C:\Users\rkvs3\.ssh\config`에는 `Host contabo14 go100 go100-contabo`, `HostName 5.104.86.14`, `User root`, `IdentityFile ~/.ssh/id_ed25519`가 설정되어 있었다.
  - CEO PC에서 `ssh -o BatchMode=yes -o ConnectTimeout=10 contabo14 "hostname && command -v codex && codex --version && command -v claude && claude --version"` 실행 성공.
- 조치:
  - `app/services/tool_executor.py`: 도구 프로세스의 로컬 PC Agent 메모리가 비어 있으면 활성 AADS API(8100/8102)로 agent 조회와 route-execute를 fallback하도록 추가했다.
  - `app/services/local_model_manager.py`: 로컬 모델 큐 상태 조회도 활성 API agent 목록을 fallback으로 사용하도록 보강했다.
- 검증:
  - `python3 -m py_compile app/services/tool_executor.py app/services/local_model_manager.py` 통과.
  - `ToolExecutor()._pc_execute()` 직접 호출이 활성 API fallback으로 CEO PC `hostname` 실행 성공.
  - `local_model_manager.queue_status(include_items=False)`가 `ready`, connected 1, capable 1로 반환.
- 남은 제한:
  - 배포 전 MCP 도구 프로세스는 컨테이너 `/app`의 이전 코드를 사용해 아직 오프라인으로 응답한다. 커밋/배포 후 MCP 도구 재검증이 필요하다.

## 2026-06-19 12:42 KST - GO100 contabo14 migration and 211 decommission gate
- 배경: CEO가 GO100 서버 정보를 AADS에 최신화한 뒤 legacy 211을 바로 폐지할 수 있는지 즉시 확인하라고 지시했다.
- 실측:
  - GO100 DB는 `5.104.86.14`에서 `kisautotrade`로 응답했다.
  - GO100 신서버는 `go100`, `go100-frontend`, PostgreSQL 16, Redis, Nginx, Kiwoom WS/뉴스/포지션/스케줄러 서비스가 active였다.
  - GO100 신서버 root crontab은 아직 없었다.
  - KIS/legacy 211은 `kis-autotrade-v4`에서 AADS runner, remote agent, nginx, redis, pgbouncer, Docker가 active였고 root crontab에 장중/장마감 자동매매, 데이터 수집, 키움 토큰 갱신, 보고서, 백업 작업이 대량 활성 상태였다.
  - KIS DB MCP 연결은 실패했지만, 211 내부 pgbouncer는 active 상태였다.
- 판정:
  - GO100 웹/API 운영 이전은 완료 상태로 본다.
  - 211 물리 폐지는 보류한다. 차단 사유는 GO100 라우팅이 아니라 KIS/공유 데이터 수집/예약작업 잔존 의존성이다.
- 조치:
  - 상세 근거와 선행 조건을 `docs/GO100-211-decommission-check-20260619.md`에 기록했다.
- 다음:
  - 211 crontab을 GO100-only, KIS-only, shared-data, backup, unsafe/destructive로 분류한다.
  - 승인된 항목만 contabo14에 중복 실행 가드와 함께 이관한다.
  - 211 cron 비활성 리허설을 1개 시장 세션 동안 수행한 뒤 물리 폐지를 재판정한다.
