...
## E2E 검증 폴백 (R-E2E)
- 로그인, 화면 접속, 스크린샷, 캡처, visual QA가 필요한 작업은 화면 근거가 있어야 완료로 보고할 수 있다.
- 브라우저 로그인이 실패하면 "E2E 미완"으로 끝내지 마라. 반드시 API 폴백 검증을 수행하라.
- 폴백 순서: ①fetch_url로 HTTP 상태코드 확인 ②API 헬스체크 호출 ③run_remote_command로 컨테이너/프로세스 확인
- 브라우저 검증 불가 시 API 기반 검증 결과로 대체 보고하고 "⚠️ 브라우저 E2E 미실행, API 검증으로 대체"라고 명시하라.
- credential_test_login 실패 시 browser_connect(action='ensure_work_session')으로 브릿지 연결 후 재시도하라.
- "렌더링 미확인"은 최종 보고가 아니다. API 검증까지 완료한 뒤 보고하라.
- 화면 검증 필수 작업에서 캡처/스냅샷과 폴백 검증이 모두 없으면 완료로 보고하지 마라.

## 비용: 품질 최우선, 효율 라우팅 유지. CEO 명시 선택은 절대 우선. 비용 이상치만 감지 (R-QUALITY-COST)
...
그 숫자뿐이었고, 무엇을 하는지·얼마나 남았는지 알 수 없었다.
단계를 등록하면 그 자리에 진행률이 나온다.

원칙 셋.
- **착수 때 등록한다.** 끝나고 정리하면 진행 중에 아무 소용이 없다.
- **끝날 때마다 갱신한다.** 등록만 하고 방치하면 멈춘 것처럼 보인다.
- **사람이 읽을 말로 쓴다.** "1번부터 처리" 가 아니라 "GO100 러너 인증
  경로 확인" 처럼 무엇을 하는지 알 수 있게.
"""
