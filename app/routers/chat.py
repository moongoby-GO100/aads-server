...
import asyncio
import structlog
from datetime import datetime, timedelta as _dt_timedelta, timezone as _dt_timezone
...
async def _settle_stale_execution_for_recovery(
    ...
    *,
    ...
    _recovery_grace = 120 if _tc > 0 else 60
    _no_db_progress = (
    ...
    _stale_empty_no_runtime = (
        ...
        and int(execution_row["updated_age_seconds"] or 0) >= 90
    )
...
# Legacy bridge endpoints were previously mounted from app.api.chat.  Keep the
# public paths here with the rest of the /chat namespace so there is one owner.
...
def _classify_legacy_chat_intent(message: str) -> Dict[str, Any]:
    msg_lower = message.lower()
    scores = []
    for rule in _LEGACY_CHAT_INTENT_RULES:
        score = sum(1 for keyword in rule["keywords"] if keyword in msg_lower)
        if score:
...
def _extract_legacy_project_spec(message: str) -> Dict[str, Any]:
    """자연어에서 프로젝트 스펙 추출 (기본)."""
    spec = {"description": message, "extracted_keywords": [], "suggested_tech": []}
    tech_map = {
    ...
    for key, value in tech_map.items():
        if key in msg_lower:
            spec["suggested_tech"].append(value)
    return spec


@router.post("/chat", response_model=LegacyChatResponse)
async def legacy_chat(req: LegacyChatRequest):
    """Legacy bridge natural-language intent router (no LLM call)."""
    logger.info("legacy_chat_received", sender=req.sender, message=req.message[:100])
    intent_result = _classify_legacy_chat_intent(req.message)
    ...
    if intent == "create_project":
        spec = _extract_legacy_project_spec(req.message)
        response_data.update({
            "message": f"프로젝트 생성 요청을 감지했습니다. 스펙: {spec['description'][:200]}",
            "api_call": {
                "method": "POST",
                "url": "/api/v1/projects",
                "body": {"description": req.message, "tech_stack": spec["suggested_tech"], "auto_run": False},
            },
            "data": spec,
        })
    elif intent == "check_status":
        response_data.update({"message": "시스템 상태를 확인합니다.", "api_call": {"method": "GET", "url": "/api/v1/health"}})
    elif intent == "check_cost":
        response_data.update({"message": "비용 현황을 조회합니다.", "api_call": {"method": "GET", "url": "/api/v1/context/system/costs"}})
    elif intent == "check_memory":
        response_data.update({"message": "경험 메모리를 조회합니다.", "api_call": {"method": "GET", "url": "/api/v1/context/experiences"}})
    elif intent == "handover":
        response_data.update({"message": "전체 시스템 현황(HANDOVER)을 생성합니다.", "api_call": {"method": "GET", "url": "/api/v1/context/handover"}})
    elif intent == "check_agents":
        response_data.update({"message": "에이전트 설정을 조회합니다.", "api_call": {"method": "GET", "url": "/api/v1/context/system/agents"}})
    elif intent == "list_projects":
        response_data.update({"message": "프로젝트 목록을 조회합니다.", "api_call": {"method": "GET", "url": "/api/v1/projects"}})
    elif intent == "check_project":
        response_data.update({"message": "프로젝트 상태를 확인합니다. 프로젝트 ID를 지정해주세요.", "api_call": {"method": "GET", "url": "/api/v1/projects"}})
    else:
        response_data["message"] = "요청을 이해했습니다. 구체적인 명령을 주시면 처리하겠습니다. 가능한 명령: 프로젝트 생성, 상태 확인, 비용 조회, 메모리 조회, 핸드오버, 에이전트 설정, 프로젝트 목록"

    return LegacyChatResponse(**response_data)
...
async def get_chat_cost_summary():
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            today = await client.get(f"{litellm_base}/spend/logs", headers={"Authorization": f"Bearer {master_key}"}, params={"start_date": str(date.today())})
            if today.status_code == 200:
                litellm_reachable = True
                daily_spend = float(today.json().get("total_cost", 0.0))
            month = await client.get(f"{litellm_base}/spend/logs", headers={"Authorization": f"Bearer {master_key}"}, params={"start_date": date.today().replace(day=1).isoformat()})
            if month.status_code == 200:
                monthly_spend = float(month.json().get("total_cost", 0.0))
                model_breakdown = month.json().get("model_breakdown", {})
    except Exception as exc:
        logger.warning("litellm_cost_summary_failed", error=str(exc))
    ...
    return {
        ...
        "today": {"cost_usd": round(daily_spend, 4), "budget_usd": daily_budget, "used_pct": round(daily_spend / daily_budget * 100, 1) if daily_budget > 0 else 0, "opus_blocked": daily_spend >= daily_budget},
        "this_month": {"cost_usd": round(monthly_spend, 4), "budget_warn_usd": monthly_budget_warn, "used_pct": round(monthly_spend / monthly_budget_warn * 100, 1) if monthly_budget_warn > 0 else 0, "over_warn": monthly_spend >= monthly_budget_warn},
...
async def get_intent_model_map():
    ...
    return {"status": "ok", "intent_model_map": INTENT_MODEL_MAP, "litellm_base_url": os.environ.get("LITELLM_BASE_URL", "http://aads-litellm:4000/v1")}


@router.get("/chat/intents")
async def list_intents():
    """사용 가능한 의도(intent) 목록 반환 - 브릿지 설정용."""
    return {"status": "ok", "intents": [{"intent": rule["intent"], "keywords": rule["keywords"], "description": rule["description"]} for rule in _LEGACY_CHAT_INTENT_RULES]}
