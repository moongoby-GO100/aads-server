    ...
    async def acquire_playwright_context(
        ...
        async with self._context_lock:
            try:
                ...
                if session and not session_id and session.endpoint.kind == BrowserEndpointKind.LOCAL_AGENT:
                    metadata = dict(session.endpoint.metadata or {})
                    if metadata.get("stale"):
                        session = None
                    else:
                        last_ok = metadata.get("last_ok_at", "")
                        if last_ok:
                            from datetime import datetime, timezone, timedelta
                            try:
                                last_ok_dt = datetime.fromisoformat(str(last_ok)) if isinstance(last_ok, str) else last_ok
                                if (datetime.now(timezone.utc) - last_ok_dt) > timedelta(hours=1):
                                    session.endpoint.metadata["stale"] = True
                                    logger.warning("acquire_playwright_context: LOCAL_AGENT %s stale (last_ok %s), headless fallback", session.session_id, last_ok)
                                    session = None
                            except Exception:
                                pass
                if session:
    ...
    async def _headless_fallback_context(self) -> Any:
        ...
        if not os.environ.get("PLAYWRIGHT_BROWSERS_PATH"):
            for candidate in ("/root/.cache/ms-playwright", "/root/.cache"):
                if any(
                    entry.startswith("chromium-") and os.path.isdir(os.path.join(candidate, entry))
                    for entry in (os.listdir(candidate) if os.path.isdir(candidate) else [])
                ):
