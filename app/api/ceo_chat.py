...
async def _handle_design_intent(
    ...
    response_text, input_tokens, output_tokens = await _call_anthropic_with_tools(
        tool_model, design_prompt, messages, dsn, chat_session_id=session_id
    )
...
async def _handle_design_fix_intent(
    ...
    analysis_text, in_tok, out_tok = await _call_anthropic_with_tools(
        tool_model, analysis_prompt, messages, dsn, chat_session_id=session_id
    )
...
async def send_ceo_message(req: CeoChatRequest):
    ...
    try:
        ...
        if intent == "qa":
            ...
            tool_model = model if model.startswith("claude") else "claude-sonnet-4-6"
            response_text, input_tokens, output_tokens = await _call_anthropic_with_tools(
                tool_model, enriched_prompt, messages, dsn, chat_session_id=session_id
            )
        elif intent == "browser":
            ...
            tool_model = model if model.startswith("claude") else "claude-sonnet-4-6"
            response_text, input_tokens, output_tokens = await _call_anthropic_with_tools(
                tool_model, pipeline_prompt, messages, dsn, max_iterations=8, chat_session_id=session_id
            )
