...
class PasswordResetRequest(BaseModel):
    ...
    @field_validator("email")
    @classmethod
    def reset_email_format(cls, v: str) -> str:
        v = v.strip().lower()
        if "@" not in v or "." not in v.split("@")[-1]:
            raise ValueError("유효하지 않은 이메일 형식입니다")
        return v
...
class PasswordResetConfirmRequest(BaseModel):
    ...
    @field_validator("password")
    @classmethod
    def reset_password_length(cls, v: str) -> str:
        if len(v) < 6:
            raise ValueError("비밀번호는 최소 6자 이상이어야 합니다")
        return v
...
async def register(req: RegisterRequest):
    ...
    auth_module.check_auth_rate_limit(scope="register", key=req.email)
    existing = await auth_module.get_saas_user_by_email(req.email)
    if existing:
        auth_module.record_auth_failure(scope="register", key=req.email)
    ...
    verification = await auth_module.create_email_verification_token(uid)
    token = auth_module.create_token(uid, user["email"], tenant_id=tenant_id)
    auth_module.record_auth_success(scope="register", key=req.email)
    ...
    return AuthResponse(
        ...
        email_verification_token=verification.get("token"),
    )
...
async def login(req: LoginRequest):
    ...
    await auth_module.require_saas_schema_ready()
    auth_module.check_auth_rate_limit(scope="login", key=req.email)
    ...
    if saas_user:
        ...
        auth_module.record_auth_success(scope="login", key=req.email)
        return AuthResponse(
            token=token,
            user_id=uid,
            ...
            is_admin=False,
            tenant_id=tenant_id,
            email_verified=bool(saas_user.get("email_verified_at")),
        )
    ...
    auth_module.record_auth_failure(scope="login", key=req.email)
    raise HTTPException(status_code=401, detail="이메일 또는 비밀번호가 올바르지 않습니다")


@router.post("/auth/logout")
async def logout(authorization: str = Header(None)):
    """현재 Bearer 토큰을 서버 측 폐기 목록에 등록한다."""
    ...
    await auth_module.revoke_token(payload, reason="logout")
    return {"ok": True}


@router.post("/auth/refresh")
async def refresh_auth_token(authorization: str = Header(None)):
    """현재 Bearer 토큰을 재발급한다. 만료 후 1시간 이내도 허용."""
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Authorization header missing")
    token = authorization[7:]
    new_token = auth_module.refresh_token(token)
    if not new_token:
...
async def verify_email(req: EmailVerifyRequest):
    """회원가입 이메일 검증 토큰을 수락한다."""
    user = await auth_module.verify_email_token(req.token)
    return {"ok": True, "user": user}


@router.post("/auth/email/resend")
async def resend_email_verification(current_user: dict = Depends(get_current_user)):
    """현재 로그인 사용자에게 이메일 검증 토큰을 재발급한다."""
    auth_module.check_auth_rate_limit(scope="email_verification", key=str(current_user.get("email") or ""))
    token = await auth_module.create_email_verification_token(str(current_user["user_id"]))
    auth_module.record_auth_success(scope="email_verification", key=str(current_user.get("email") or ""))
    return {"ok": True, "email_verification_token": token.get("token"), "expires_at": token.get("expires_at")}


@router.post("/auth/password-reset/request")
async def request_password_reset(req: PasswordResetRequest):
    """비밀번호 재설정 요청. 존재하지 않는 이메일도 동일 응답으로 처리한다."""
    auth_module.check_auth_rate_limit(scope="password_reset", key=req.email)
    await auth_module.create_password_reset_token(req.email)
    auth_module.record_auth_success(scope="password_reset", key=req.email)
    return {"ok": True}


@router.post("/auth/password-reset/confirm")
async def confirm_password_reset(req: PasswordResetConfirmRequest):
    """비밀번호 재설정 토큰으로 새 비밀번호를 적용한다."""
    user = await auth_module.reset_password_with_token(req.token, req.password)
    return {"ok": True, "user_id": str(user.get("id")), "email": user.get("email")}
...
async def get_me(current_user: dict = Depends(get_current_user)):
    ...
    return {
        ...
        "email_verified": bool(current_user.get("email_verified_at") or current_user.get("is_admin")),
        "email_verified_at": current_user.get("email_verified_at").isoformat() if current_user.get("email_verified_at") else None,
