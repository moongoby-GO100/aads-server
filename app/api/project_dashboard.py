...
async def complete_running_tasks(project: Optional[str] = None):
    """진행중 상태인 task를 일괄 완료(completed) 처리 — 스턱 태스크 정리"""
    updated = 0
    try:
        async with memory_store.pool.acquire() as conn:
            if project and project != "all":
                rows = await conn.fetch(
                    "SELECT task_id, source FROM project_tasks WHERE status = 'running' AND project = $1",
                    project
                )
                result = await conn.execute(
                    "UPDATE project_tasks SET status='completed', completed_at=NOW() WHERE status='running' AND project=$1",
                    project
                )
            else:
                rows = await conn.fetch(
                    "SELECT task_id, source FROM project_tasks WHERE status = 'running'"
                )
                result = await conn.execute(
                    "UPDATE project_tasks SET status='completed', completed_at=NOW() WHERE status='running'"
                )
    ...
    logger.info(f"complete_running: {updated}건 완료처리 (project={project or 'all'})")
    return {"status": "ok", "updated": updated, "message": f"{updated}건이 완료 처리되었습니다."}
