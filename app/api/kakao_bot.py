...
async def agent_version():
    ...
    # min_version: pc_agent/MIN_VERSION 파일 읽기, 없으면 기본값
    min_version_file = PC_AGENT_DIR / "MIN_VERSION"
    min_version = "1.0.0"
    if min_version_file.exists():
        min_version = min_version_file.read_text(encoding="utf-8").strip()

    # force_update: pc_agent/FORCE_UPDATE 파일이 존재하고 "true" 이면 True
    force_update_file = PC_AGENT_DIR / "FORCE_UPDATE"
    force_update = False
    if force_update_file.exists():
        force_update = force_update_file.read_text(encoding="utf-8").strip().lower() == "true"

    # changelog: pc_agent/CHANGELOG 파일 읽기, 없으면 기본 문구
    changelog_file = PC_AGENT_DIR / "CHANGELOG"
    changelog = "v1.1.0: API 기반 자동 업데이트, 다중 PC 지원, 안정성 개선"
