...
EMAIL_VERIFICATION_EXPIRE_HOURS = 24 * 7
AUTH_RATE_LIMIT_WINDOW_SECONDS = int(os.getenv("AADS_AUTH_RATE_LIMIT_WINDOW_SECONDS", "900"))
AUTH_RATE_LIMIT_MAX_ATTEMPTS = int(os.getenv("AADS_AUTH_RATE_LIMIT_MAX_ATTEMPTS", "8"))
...
def refresh_token(token: str) -> Optional[str]:
    """Re-issue JWT with fresh expiry. Allows refresh within 1h after expiry."""
    if not JWT_AVAILABLE:
        return None
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        ...
        try:
            ...
            if time.time() - payload.get('exp', 0) > REFRESH_GRACE_SECONDS:
                return None
            return create_token(
                payload['sub'],
                payload.get('email', ''),
...
async def authenticate_saas_user(email: str, password: str) -> Optional[dict]:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                """SELECT id, email, name, password_hash, default_tenant_id, role, email_verified_at
                     FROM saas_users
                    WHERE email = $1
...
async def get_saas_user_by_email(email: str) -> Optional[dict]:
    try:
        ...
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                """SELECT id, email, name, default_tenant_id, role, email_verified_at
                     FROM saas_users
                    WHERE email = $1
                      AND deleted_at IS NULL""",
                email
...
def check_auth_rate_limit(*, scope: str, key: str) -> None:
    normalized = f"{scope}:{_normalize_email(key)}"
    now = time.time()
    window_start = now - AUTH_RATE_LIMIT_WINDOW_SECONDS
    attempts = [ts for ts in _AUTH_ATTEMPTS[normalized] if ts >= window_start]
    _AUTH_ATTEMPTS[normalized] = attempts
...
def record_auth_failure(*, scope: str, key: str) -> None:
    normalized = f"{scope}:{_normalize_email(key)}"
    _AUTH_ATTEMPTS[normalized].append(time.time())


def record_auth_success(*, scope: str, key: str) -> None:
    normalized = f"{scope}:{_normalize_email(key)}"
    _AUTH_ATTEMPTS.pop(normalized, None)


async def require_auth_security_schema_ready() -> None:
    pool = await _ensure_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            SELECT
                ...
                to_regclass('public.password_reset_tokens') IS NOT NULL AS has_password_reset_tokens,
                to_regclass('public.email_verification_tokens') IS NOT NULL AS has_email_verification_tokens,
                to_regclass('public.auth_revoked_tokens') IS NOT NULL AS has_auth_revoked_tokens
            """
        )
    missing = [
        name
        for name in (
            "has_email_verified",
            "has_password_reset_tokens",
            "has_email_verification_tokens",
            "has_auth_revoked_tokens",
        )
        if not row or not row[name]
    ]
    if missing:
        log.error("auth_security_schema_not_ready", missing=missing)
        raise HTTPException(status_code=503, detail="Authentication security schema is not initialized")
...
async def create_email_verification_token(user_id: str) -> dict:
    await require_auth_security_schema_ready()
    token = secrets.token_urlsafe(32)
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            VALUES ($1::text, $2, now() + ($3::text || ' hours')::interval)
            RETURNING id::text AS token_id, user_id, expires_at, created_at
            """,
            user_id,
            token_hash,
...
async def verify_email_token(token: str) -> dict:
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            row = await conn.fetchrow(
                """
                SELECT id::text AS token_id, user_id, expires_at, used_at
            ...
            user = await conn.fetchrow(
                "SELECT id, email, name, email_verified_at FROM saas_users WHERE id = $1::text",
                row["user_id"],
            )
    return dict(user) if user else {"user_id": row["user_id"]}


async def create_password_reset_token(email: str, *, requested_by: Optional[str] = None) -> Optional[dict]:
    await require_auth_security_schema_ready()
    normalized_email = _normalize_email(email)
    pool = await _ensure_pool()
    async with pool.acquire() as conn:
        ...
        row = await conn.fetchrow(
            ...
            VALUES (
                ...
                $2,
                CASE
                    WHEN EXISTS(SELECT 1 FROM saas_users WHERE id = $3::text) THEN $3::text
                    ELSE NULL
                END,
...
async def reset_password_with_token(token: str, new_password: str) -> dict:
    ...
    token_hash = _hash_auth_token(token)
    password_hash = _bcrypt_mod.hashpw(new_password.encode("utf-8"), _bcrypt_mod.gensalt()).decode("utf-8")
    pool = await _ensure_pool()
    async with pool.acquire() as conn:
        async with conn.transaction():
            row = await conn.fetchrow(
                """
                SELECT id::text AS token_id, user_id, expires_at
            ...
            await conn.execute(
                """
                UPDATE auth_revoked_tokens
                   SET revoked_reason = COALESCE(NULLIF(revoked_reason, ''), 'password_reset')
...
async def revoke_token(payload: dict, *, reason: str = "logout") -> None:
    await require_auth_security_schema_ready()
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            """
            INSERT INTO auth_revoked_tokens (jti, user_id, expires_at, revoked_reason)
            VALUES ($1, $2, $3, $4)
...
async def is_token_revoked(payload: Optional[dict]) -> bool:
    if not payload:
        return True
    jti = str(payload.get("jti") or "").strip()
    if not jti:
    ...
    try:
        ...
        log.warning("token_revocation_check_failed", error=str(e))
        return False


async def set_internal_admin_access(user_id: str, *, enabled: bool, updated_by: str) -> dict:
    await require_saas_schema_ready()
    pool = await _ensure_pool()
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            if not user:
                raise HTTPException(status_code=404, detail="User not found")
            internal_tenant_id = await conn.fetchval("SELECT public.aads_internal_tenant_id()")
            if not internal_tenant_id:
                raise HTTPException(status_code=503, detail="Internal tenant unavailable")
            if enabled:
                role = "admin" if str(user["role"] or "").lower() not in {"ceo", "system"} else str(user["role"])
                await conn.execute(
                    """
                    UPDATE saas_users
                       SET role = $2,
                           default_tenant_id = $3::uuid,
                           updated_at = now()
                     WHERE id = $1::text
                    """,
                    user_id,
                ...
                membership = await conn.fetchrow(
                    """
                    INSERT INTO tenant_memberships (tenant_id, user_id, role, status, invited_by)
                    VALUES ($1::uuid, $2::text, 'admin', 'active', $3::text)
                    ON CONFLICT (tenant_id, user_id) DO UPDATE
                       SET role = CASE
                               WHEN tenant_memberships.role = 'owner' THEN 'owner'
                               ELSE 'admin'
                ...
                await conn.execute(
                    """
                    UPDATE saas_users
                       SET role = CASE WHEN role = 'admin' THEN 'user' ELSE role END,
                           default_tenant_id = (
                               SELECT tm.tenant_id
                                 FROM tenant_memberships tm
                                 JOIN tenants t ON t.id = tm.tenant_id
                                WHERE tm.user_id = saas_users.id
                                  AND tm.status = 'active'
                                  AND tm.deleted_at IS NULL
                                  AND t.kind = 'customer'
                                  AND t.deleted_at IS NULL
    ...
    result = dict(updated)
    result["membership"] = dict(membership) if membership else None
    return result


async def backfill_chat_session_user_ids() -> dict:
    await require_saas_schema_ready()
    pool = await _ensure_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch(
...
async def get_current_user(
    ...
    current_user['email_verified_at'] = context.get('email_verified_at')
