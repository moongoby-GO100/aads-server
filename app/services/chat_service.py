...
async def _archive_interrupted_siblings_for_completed_execution(
    conn: asyncpg.Connection,
    ...
    *,
    ...
    status = await conn.execute(
        ...
        SET intent = '_archived_partial',
            quality_details = COALESCE(quality_details, '{}'::jsonb)
                || jsonb_build_object(
                    'archived_reason', $3::text,
                    'archived_at', NOW(),
                    'final_message_id', $2::text
                ),
            edited_at = NOW()
        WHERE execution_id = $1
          AND id <> $2
    ...
    try:
        archived_count = int(str(status).split()[-1])
    except Exception:
        archived_count = 0
    if archived_count:
        logger.warning(
            "completed_execution_archived_interrupted_siblings execution=%s final=%s count=%s reason=%s",
            str(execution_id)[:8],
            str(final_message_id)[:8],
            archived_count,
            reason,
        )
    ...
    *,
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            # "정상 응답" = model_used가 실제 모델명(streaming/recovered/stopped/interrupted 제외)
            _last_user_before = await conn.fetchval(
                """SELECT created_at FROM chat_messages
                   WHERE session_id = $1 AND role = 'user'
                     AND created_at < $2
                   ORDER BY created_at DESC LIMIT 1""",
                uuid.UUID(session_id), placeholder['created_at'],
            )
            if _last_user_before:
                # 해당 user 메시지 이후에 정상 저장된 assistant 응답이 있는지 확인
                final_exists = await conn.fetchval(
                    ...
                    uuid.UUID(session_id), placeholder['id'], _last_user_before,
                )
            else:
                # user 메시지가 없으면 기존 방식으로 폴백 (placeholder 이후 assistant 응답 확인)
                final_exists = await conn.fetchval(
                    """SELECT count(*) FROM chat_messages
                        WHERE session_id = $1 AND role = 'assistant'
                        AND intent IS DISTINCT FROM 'streaming_placeholder'
                        AND id != $2
                        AND created_at >= $3""",
                    uuid.UUID(session_id), placeholder['id'], placeholder['created_at'],
                )
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                await _archive_interrupted_siblings_for_completed_execution(
                    ...
                    execution_uuid,
                    assistant_message_id,
                    reason="completion_guard_marked_completed",
                )
                logger.info(
        ...
        async with _RESUME_SEMAPHORE:
            ...
            if not _resume_model:
                try:
                    ...
                    _resume_model = await _get_default_llm_model_from_db()
...
def get_streaming_status(session_id: str, acked_completion_token: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if session_id in _streaming_state:
        ...
        # 단, 명시적으로 중단 보존된 터미널 메시지는 화면 reload 신호를 유지한다.
        _completion_signal_allowed = bool(s.get("saw_done_event")) or _looks_terminal_interrupt_content(_content)
        _emit_just_completed = is_completed and _completion_signal_allowed
...
async def send_message_stream(
    ...
    try:
        ...
        if not model_override or str(model_override).strip() in ("mixture", "auto", "") or _auto_default_requested:
            ...
            except Exception as _db_default_err:
                logger.warning(f"auto_route_db_default_lookup_failed session={session_id[:8]}: {_db_default_err}")
                _db_default_model = None
            if _db_default_model:
                logger.info(
                    "auto_route_db_default_applied session=%s intent=%s model=%s -> %s",
                    session_id[:8],
                    intent,
                    intent_result.model,
                    _db_default_model,
        ...
        if not _validation.is_valid and response_mode == _RESPONSE_MODE_FAST:
            logger.warning(
                f"output_validator_fast_mode_bypass: {_validation.violation_type} — "
                f"{_validation.message} (intent={intent}, model={model_used}, tokens_out={output_tokens})"
            )
        elif not _validation.is_valid:
            logger.warning(
        ...
        try:
            if _should_enforce_completion_contract(response_mode):
                ...
                logger.info("completion_contract_skipped_fast_mode session=%s", session_id[:8])
