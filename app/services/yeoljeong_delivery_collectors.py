...
def normalize_record(
    ...
    occurred_on = _date(
        _first(row, ("거래일", "주문일", "매출일", "정산일", "리뷰일", "작성일", "date", "일자"))
    )
    record: dict[str, Any] = {
        ...
        record.update(
            {
                ...
                "settlement_amount": _number(_first(row, ("정산금액", "지급금액", "입금액"))),
                "settlement_status": _first(row, ("정산상태", "지급상태", "상태")),
            }
        )
    else:
        record.update(
...
def _click_first(page: Any, labels: tuple[str, ...], timeout: int = 2500) -> bool:
    for label in labels:
        try:
            locator = page.get_by_text(re.compile(re.escape(label), re.I), exact=False).first
...
def _collect_section(page: Any, labels: tuple[str, ...], date_from: str, date_to: str, download_dir: Path) -> tuple[list[dict[str, Any]], str]:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            for kind, labels in config["sections"].items():
                rows, source = _collect_section(page, labels, date_from, date_to, temp_dir)
                collected[kind] = [
