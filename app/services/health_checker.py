...
async def check_pipeline_status() -> Dict[str, Any]:
    ...
    async def _check_remote_211() -> Dict:
        ...
        if not output or output.startswith("(error"):
            return {"reachable": False, "error": output}
        result = {}
...
async def _check_ssh(server_key: str) -> Dict[str, Any]:
    ...
    ok = "ok" in output
    result = {"ok": ok, "latency_ms": latency}
    if not ok:
        result["error"] = output[:200] if output else "timeout or unreachable"
    return result
...
async def _check_memory() -> Dict[str, Any]:
    ...
    parts = output.split()
    if len(parts) < 3:
        return {"ok": False, "error": "unexpected free output"}
    try:
        total_mb = int(parts[1])
        used_mb = int(parts[2])
        return {"ok": True, "used_mb": used_mb, "total_mb": total_mb}
...
async def _check_cpu() -> Dict[str, Any]:
    ...
    load_avg = output.strip().split(",")[0].strip()
    try:
        load_val = float(load_avg)
        return {"ok": load_val < 4.0, "load_avg": load_avg}
    except ValueError:
        return {"ok": True, "load_avg": load_avg}
...
async def check_infra() -> Dict[str, Any]:
    ...
    for i, key in enumerate(keys):
        if isinstance(results[i], Exception):
            ...
            issues.append({"type": f"{key}_error", "detail": str(results[i])})
        else:
            infra[key] = results[i]
