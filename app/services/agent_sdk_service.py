...
_TOOL_GRADES: Dict[str, str] = {
    ...
    "edit_image":           "Yellow",
    "generate_video":       "Yellow",
    "video_status":         "Green",
    "video_download":       "Yellow",
...
def _build_aads_sdk_tools() -> list:
    ...
    tools = [
        ...
        # AADS media generation tools
        _wrap("generate_image", "이미지 생성 — 공통 media_generation_jobs job 기록 포함 (Yellow)",
              {"prompt": str, "size": str, "model_id": str, "provider": str}),
        _wrap("edit_image", "이미지 편집 — gpt-image-2 라우팅/NOT_CONFIGURED graceful 반환 (Yellow)",
              {"prompt": str, "image_path": str, "image_url": str, "mask_path": str, "size": str, "model_id": str}),
        _wrap("generate_video", "비동기 동영상 생성 job 생성 — sora-2/veo-3.1 라우팅 인식 (Yellow)",
              {"prompt": str, "input_refs": dict, "model_id": str, "provider": str}),
        _wrap("video_status", "동영상 생성 job 상태 조회",
              {"job_id": str}),
        _wrap("video_download", "완료 동영상 결과 저장/메타데이터 반환 (Yellow)",
