...
overall = factual_grounding×0.35 + completeness×0.25 + actionability×0.20 + ceo_alignment×0.20

## 응답 형식 (JSON만):
{{"factual_grounding": 0.0~1.0, "completeness": 0.0~1.0, "actionability": 0.0~1.0, "ceo_alignment": 0.0~1.0, "overall": 0.0~1.0, "feedback": "재생성 필요 시 구체적 개선 지시", "note": "한줄 평가"}}
...
async def critique_response(
    ...
    try:
        ...
        overall = (
            ...
            + float(details.get("actionability", 0.5)) * 0.20
            + float(details.get("ceo_alignment", 0.5)) * 0.20
        )
