    ...
    try:
        ...
        async with pool.acquire() as conn, conn.transaction():
            ...
            if resolved_milestone:
                milestone_status = await conn.fetchval(
                ...
                elif blocked_remediation and milestone_status not in (
                    "pending", "in_progress", "blocked"
                ):
                    logger.warning(
                        "goal_link_rejected_closed_remediation_milestone job=%s milestone=%s status=%s",
                        job_id, resolved_milestone, milestone_status,
                    )
                    return None
            elif goal["status"] == "blocked":
                logger.warning(
                    "goal_link_rejected_milestone_required job=%s goal=%s",
                    job_id, binding.goal_id,
                )
                return None
    ...
    async def _run_inner(self):
        ...
        try:
            ...
            if _wm == "litellm" or _wm.startswith("litellm:"):
                # CEO 명시 지정: 해당 모델 직행
                _direct_model = _wm.split(":", 1)[1] if ":" in _wm else _LITELLM_FALLBACK_MODELS.get(self.size, "groq-llama-70b")
