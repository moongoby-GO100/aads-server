...
CASES = [
    ...
    ("확실하게 루프로 진행요청만 감지 할수 있게 수정 조치해", None, "루프 감지 구현 지시 — 오탐 차단"),
    (
    ...
    ("완료시까지 진행하고 보고해", None, "완료시까지 일반 진행 지시 — 오탐 차단"),
    ("끝날 때까지 계속 진행해", None, "계속 진행 일반 지시 — 오탐 차단"),
...
loop_chat_handler._remember_pending("verify-loop-confirm", "서버 상태 10분마다 감시해")
got = detect_loop_intent("루프 진행", session_id="verify-loop-confirm")
ok = got == "loop_start"
if not ok:
    fail += 1
print(f"{'PASS' if ok else 'FAIL'} | expected={'loop_start':12} got={got!s:12} | 정탐: 대기 확인 승인")
