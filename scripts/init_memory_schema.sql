...
INSERT INTO ai_observations (category, key, value, confidence, project) VALUES
  ...
  ('project_pattern', 'kis_workdir', 'KIS 작업 디렉터리: /root/webapp (서버211). 소스 경로 project_config.py 참조.', 0.7, 'KIS'),
  ...
  ('project_pattern', 'kis_systemd', 'KIS systemd 서비스: kis-autotrade-api.service (uvicorn 8000, WorkingDirectory=/root/webapp/backend), integrated-trading.service (integrated_trading_system.py), kis-autotrade-scalping.service. 모두 cwd=/root/webapp/backend 에서 실행.', 0.8, 'KIS'),
  ...
  ('project_pattern', 'go100_workdir', 'GO100 작업 디렉터리: /root/go100 (서버211). 소스 경로 project_config.py 참조.', 0.7, 'GO100'),
  ('project_pattern', 'go100_source_map', 'GO100 서비스별 소스 경로: KIS와 동일 구조 (fork). [API서버] backend/app/main.py. [라우터] backend/app/api/v1/. [전략] backend/app/services/strategy/. [KIS API] backend/app/services/kis/. [모델] backend/app/models/. [프론트엔드] frontend/. Docker Compose로 배포 (docker-compose.yml: backend + frontend 서비스). 현재 서비스 미가동 상태.', 0.8, 'GO100'),
