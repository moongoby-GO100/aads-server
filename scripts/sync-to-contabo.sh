...
if [ -n "$DASHBOARD_HASH" ] && [ "$DASHBOARD_HASH" != "$DASHBOARD_OLD_HASH" ]; then
  ...
  if ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o ConnectTimeout=10 $CONTABO "cd /root/aads/aads-dashboard && DASHBOARD_EXTERNAL_HEALTH_URL=http://127.0.0.1/login bash deploy.sh" >> $LOG 2>&1; then
