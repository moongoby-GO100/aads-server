#!/usr/bin/env python3
"""주식회사 윤희에프엔비 법인 설립 서류 — 보정명령 반영 수정본 (v2)
보정명령 접수번호: 제455534호 (2026-06-16, 서울중앙지방법원 등기국)
수정사항:
  1) 대표이사 → 사내이사 (1인 이사 회사 = 대표이사 없음)
  2) 공고방법: 인터넷 삭제, 신문사 특정
  3) 발기인총회의사록: 대표이사 선임 의안 삭제, 감사 선임결의 명확화
  4) 취임승낙서: 대표이사 페이지 삭제 → 사내이사 + 감사 2종
  5) 인감신고서/주주명부: 대표이사 → 사내이사
"""
import os
import shutil
...
def gen_articles():
    ...
    articles = [
        ('제1장  총  칙', [
            ...
            ('제4조 (공고방법)', '본 회사의 공고는 서울특별시에서 발행되는 일간 서울경제에 게재한다.'),
        ]),
        ('제2장  주  식', [
            ...
            ('제10조 (주식의 양도제한)', '본 회사의 주식을 양도하고자 할 때에는 이사의 승인을 받아야 한다.'),
            ('제11조 (명의개서대리인)', '① 본 회사는 주식의 명의개서대리인을 둘 수 있다.\n② 명의개서대리인 및 그 사무취급장소와 대행업무의 범위는 이사가 결정한다.'),
        ]),
        ('제3장  사  채', [
            ('제12조 (사채의 발행)', '본 회사는 이사의 결정에 의하여 사채를 발행할 수 있다.'),
        ]),
        ('제4장  주주총회', [
            ('제13조 (소집시기)', '① 본 회사의 정기주주총회는 매 사업연도 종료 후 3월 이내에 소집한다.\n② 임시주주총회는 필요에 따라 수시로 소집한다.'),
            ('제14조 (소집권자)', '주주총회는 법령에 다른 규정이 있는 경우를 제외하고는 사내이사가 소집한다.'),
            ('제15조 (소집통지 및 공고)', '주주총회를 소집함에는 그 일시, 장소 및 회의의 목적사항을 총회일 2주 전에 각 주주에게 서면 또는 전자문서로 통지를 발송하여야 한다.'),
            ('제16조 (의장)', '주주총회의 의장은 사내이사로 한다.'),
        ...
        ('제5장  이  사', [
            ...
            ('제24조 (회사의 대표)', '이사가 1인인 경우에는 그 이사가 회사를 대표하며 회사의 업무를 집행한다.'),
        ]),
        ('제6장  감  사', [
            ('제25조 (감사의 선임)', f'① 본 회사는 감사 1인을 둔다.\n② 감사는 주주총회에서 선임한다.\n③ 감사의 임기는 취임 후 3년 내의 최종의 결산기에 관한 정기주주총회 종결 시까지로 한다.'),
        ]),
        ('제7장  계  산', [
            ('제26조 (사업연도)', '본 회사의 사업연도는 매년 1월 1일부터 12월 31일까지로 한다.'),
            ('제27조 (재무제표의 작성 등)', '① 사내이사는 매 사업연도 종료 후 다음 각호의 서류와 그 부속명세서 및 영업보고서를 작성하여야 한다.\n  1. 대차대조표\n  2. 손익계산서\n  3. 이익잉여금처분계산서 또는 결손금처리계산서'),
            ('제28조 (이익배당)', '① 이익배당은 금전으로 한다.\n② 이익의 배당을 주주총회의 결의에 의하여 주주에게 지급한다.\n③ 이익배당은 매 결산기말 현재의 주주명부에 기재된 주주 또는 등록된 질권자에게 지급한다.'),
        ]),
        ('제8장  부  칙', [
            ('제29조 (설립비용)', '본 회사의 설립에 소요되는 비용은 금 삼백만원 이내로 한다.'),
            ('제30조 (발기인의 성명과 주소 및 인수주식)', f'본 회사 발기인의 성명, 주민등록번호, 주소 및 인수 주식수는 다음과 같다.\n\n  발기인: {REP}\n  주민등록번호: {SSN}\n  주소: {ADDR}\n  인수 주식수: {ISSUE_SHARES:,}주 (금 {CAPITAL:,}원)'),
            ('제31조 (최초 사업연도)', '본 회사의 최초 사업연도는 회사 설립등기일로부터 2026년 12월 31일까지로 한다.'),
        ]),
    ]
...
# ============================================================
# 2. 발기인총회의사록 (보정: 대표이사 선임 삭제, 사내이사로 변경)
# ============================================================
def gen_founders_minutes():
    ...
    agendas = [
        ...
        ('제2호 의안: 사내이사 선임의 건',
         f'의장이 설립시 사내이사 선임을 제안하고, 다음과 같이 사내이사를 선임하기로 만장일치 가결하다.\n\n  사내이사: {REP} (주민등록번호: {SSN})\n  주소: {ADDR}\n\n  위 사내이사가 1인이므로 상법 제389조 제1항 단서에 의하여 회사를 대표한다.'),
        ('제3호 의안: 감사 선임의 건',
         f'의장이 설립시 감사 선임을 제안하고, 다음과 같이 감사를 선임하기로 만장일치 가결하다.\n\n  감사: {AUDITOR} (주민등록번호: {AUDITOR_SSN})\n  주소: {AUDITOR_ADDR}'),
        ('제4호 의안: 본점소재지 결정의 건',
         f'본점을 {HQ}에 두기로 만장일치 가결하다.'),
        ('제5호 의안: 설립비용 및 발기인 보수의 건',
...
# ============================================================
# 4. 조사보고서 (감사 오병용 — 전자서명은 CEO/감사 직접 처리)
# ============================================================
...
# ============================================================
# 5. 취임승낙서 (보정: 대표이사 삭제 → 사내이사 + 감사 2종만)
# ============================================================
def gen_acceptance():
    doc = new_doc()
    ...
    add_p(doc, '(사내이사)', size=14, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, after=20)

    add_p(doc, f'본인은 2026년    월    일 개최된 {CORP} 발기인총회에서 사내이사로 선임되었기에 이를 승낙합니다.', size=11, after=30)

    add_p(doc, '2026년      월      일', size=11, align=WD_ALIGN_PARAGRAPH.CENTER, after=30)
    add_p(doc, '사내이사 취임자', size=12, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, after=10)
...
def gen_shareholder_list():
    ...
    add_p(doc, '2026년      월      일', size=11, align=WD_ALIGN_PARAGRAPH.CENTER, after=30)
    add_p(doc, CORP, size=12, align=WD_ALIGN_PARAGRAPH.CENTER, after=10)
    add_p(doc, f'사내이사   {REP}  (인)', size=12, align=WD_ALIGN_PARAGRAPH.CENTER, after=10)
...
def gen_seal_report():
    ...
    add_p(doc, '', after=20)
    add_p(doc, '위 법인의 사내이사로서 아래의 인감을 법인 인감으로 신고합니다.', size=11, after=20)
    ...
    add_p(doc, '개인 인감 (사내이사)', size=14, bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, after=10)
    add_p(doc, '┌─────────────────────┐', size=12, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
    add_p(doc, '│                                              │', size=12, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
    add_p(doc, '│          (인감 날인란)              │', size=12, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
    add_p(doc, '│                                              │', size=12, align=WD_ALIGN_PARAGRAPH.CENTER, after=0)
...
if __name__ == '__main__':
    print('=' * 60)
    print(f'{CORP} 법인 설립 서류 7종 — 보정명령 반영 수정본')
    print('보정 접수번호: 제455534호')
    print('=' * 60)
    results = []
    ...
    for r in results:
        ...
        alias = ASCII_ALIASES.get(fname, '')
        dst = os.path.join(STATIC_DIR, fname)
        shutil.copy2(r, dst)
        if alias:
            shutil.copy2(r, os.path.join(STATIC_DIR, alias))
        print(f'  -> {dst}')
    print('=' * 60)
