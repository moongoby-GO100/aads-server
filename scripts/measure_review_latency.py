...
runner-1b282742 의 실제 diff(48,360B)로 등록된 리뷰 모델 각각의 응답 시간을 잰다.
DB 에 아무것도 쓰지 않는다 — _call_review_model 만 직접 부른다.
...
async def main() -> int:
    ...
    print(f"job={job_id} diff_bytes={len(diff)} prompt_diff_bytes={len(truncated)} truncated={was_cut}")
    print(f"models={models}")
    print(f"current limits: per_attempt={cr._REVIEW_LLM_TIMEOUT_SEC}s "
    ...
    for model in models:
        ...
        try:
            text, _ = await asyncio.wait_for(
                cr._call_review_model(model, prompt), timeout=probe_timeout
            )
        ...
        except Exception as exc:  # noqa: BLE001 - 측정 스크립트, 원문을 그대로 보여준다
            elapsed = time.monotonic() - started
            print(f"  {model:32s} {elapsed:7.1f}s ERROR {str(exc)[:120]}")
    return 0
