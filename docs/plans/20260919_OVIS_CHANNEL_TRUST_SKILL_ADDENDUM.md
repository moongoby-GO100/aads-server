...
등록된 실행 순서: M3 기존 인증 검수 → G1 채널 라우터 → G2 신뢰 경계 → G3 스킬 함수 → G4 ARIA → G5 사실 재검증 → G6 골든 승격 → M4~M11 기존 통합·E2E·배포. G3~G6은 먼저 공용 계약을 구현하고 후속 M7~M11이 이를 실제 사이트 학습·검색·화면에 연결한다. 독립 모듈 테스트만 통과한 상태는 최종 결선 완료가 아니다.
