...
완료된 AI 응답은 DB 트리거가 `tenant + project + session` 단위 체크포인트로 자동 갱신한다. 동일 세션은 현재 항목 1건만 유지하고 변경마다 리비전 이벤트를 남긴다. 에이전트는 장기 보존할 결정·위험·검증을 `handover_write`로 별도 구조화하며, 사용자는 `handover_search` 또는 REST 조회로 프로젝트·상태·유형·키워드를 검색한다.
...
- 모든 정상 최종 AI 응답은 `session:<session_uuid>` 안정 키로 자동 체크포인트를 남긴다. 스트리밍 placeholder·중단 partial은 정본으로 승격하지 않는다.
...
8. 기존 세션은 최신 정상 AI 응답으로 1회 백필되고, 신규 응답은 별도 에이전트 도구 호출 없이 리비전이 증가한다.
