...
- Change: `nginx-fb.conf` routes `/login` to `/static/apps/obys/index.html` while preserving `redirect`. The FB store assistant app stores the recipe redirect and returns to `/unni-naengmyeon/recipes` after successful login/signup.
...
- Verification:
  ...
  - `https://unni.newtalk.kr/unni-naengmyeon/recipes` now redirects to `https://fb.newtalk.kr/static/apps/obys/index.html?redirect=%2Funni-naengmyeon%2Frecipes`.
