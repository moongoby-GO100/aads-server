...
- 이미 이전 코드가 `retry_count=5` 실행을 `execution_resume_attempt_limit_exceeded`로 닫아 둔 legacy row는 수동 `/resume`에서 retry count를 더 올리지 않고 5번째 시도를 복구한다. 그 외 `retry_count=5` 중단은 기존 cap 정책을 따른다.
