...
6. Candidate health must execute `/usr/local/bin/claude-aads --version` inside the actual candidate container and reject any version below 2.1.280 before routing changes. A passing host-global or runner CLI audit cannot substitute for this check.
7. Release provenance must include the compatible-CLI commit. A divergent release branch must be reconciled with the canonical release line before deployment; queued API releases use the scoped fast-forward preflight. Direct operators must verify ancestry as well as the candidate image.

## 2026-09-23 recurrence

Session `1fa84036-b12d-4497-97f5-076a32645a20` reproduced the HTTP 400 after image `9106d5c8` was deployed from a branch that did not descend from the compatible-CLI release. Both API slots ran SDK-bundled Claude Code 2.1.259, despite host and runner 2.1.280. The candidate-image version gate is therefore a release-blocking acceptance criterion, not just an audit after cutover.
