...
| 파일 저장소 해시·용량 | 완료 (19절) — 952파일 65,898,759B, 해시 대조 952/952 |
| 전환 시간·복원 예산 | 부분 완료 (19절) — 데이터 동기화 12.06초 실측, 앱 기동·라우팅 전환 미측정 |
...
## 19. M0 기준선 확정 (2026-09-23 18:33~18:47 KST 실측)

17.5절이 남긴 두 항목을 닫고 포함/제외 manifest·담당·개별 PRD 상태를 확정했다.
전문은 `docs/reports/20260923_OBYS_M0_MIGRATION_BASELINE.md`.

| 완료 기준 | 상태 |
|---|---|
| 포함/제외 manifest | 확정 — API 116, `obys` 40테이블 39MB, 인증 7테이블, 파일 952개 65.9MB, 조직 2·사업자 5·사용자 6 |
| 소스 SHA | **미확정** — 운영 프로세스가 bind mount 된 dirty 워킹트리를 실행 중 |
| 용량·복사/복원 예산 | 확정 — 덤프·전송·복원·파일동기화 합계 12.06초, 해시 952/952 일치 |
| RTO 예산 | **부분 확정** — 데이터 구간만 실측, 앱 기동·라우팅 전환 미측정 |
| 역할별 담당 | **부분 확정** — `owner_role_key` 는 배정(M0·M5·M6 CTO / M1~M4 Developer)이나 7건 모두 `owner_session_id` NULL |
| 개별 PRD | **미확정** — M1~M6 개별 PRD 미작성. 목표 연결 정본은 v1.1 인데 커밋본은 v1.2 로 불일치 |

실측으로 새로 드러난 차단 요인 6건(3절 가정 교정 포함):

1. 운영 코드가 릴리스가 아니다 — 컨테이너의 `obys_upload_service.UPLOAD_ROOT` 가
   고정 문자열인데 origin/main 은 `os.getenv("OBYS_UPLOAD_ROOT", …)` 다. 기준선 SHA 고정 불가(M1).
2. 인증 7테이블 전량 복원은 금지 — 대상 6명이 타 AADS 조직 멤버십 6건을 갖는다(M2).
3. 공유 인증 DB 가 반일 만에 14행 늘었다 — 공통 쓰기 장벽 없이는 유실된다(M5).
4. 수집이 분리 워커가 아니라 API 프로세스 안에서 파일 lock 으로 돈다.
   compose 의 `yeoljeong-finance-worker` 컨테이너는 존재하지 않는다(M3).
5. 업무 파일과 DB 가 이중 보관이다 — `delivery_*.json` 과 `yeoljeong_delivery_*` 정본 미확정(M3).
6. `YEOLJEONG_FINANCE_DATABASE_URL` 이 빈 값이라 인증 DB 로 폴백한다(M1).

용량은 제약이 아니다. 전환 시간을 지배하는 것은 복사가 아니라 쓰기 동결·검증·라우팅 절차다.
