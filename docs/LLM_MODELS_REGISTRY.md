...
**마지막 업데이트:** 2026-04-28 10:30 KST | **버전:** v5.0 | **작성:** AADS-LLM-AUTO
...
- 자동 발견
  - `app/services/model_registry.py`가 OpenAI, Gemini, LiteLLM catalog를 조회해 `llm_models`에 병합한다.
  - `llm_model_discovery_runs`에 provider별 발견 결과, 활성화 수, 오류/skip 사유를 기록한다.
  - OpenAI/Gemini는 실행 가능한 텍스트 모델만 자동 `is_selectable/is_executable=true`로 노출하고, 나머지는 `review_required`로 남긴다.
  - Anthropic은 현재 AADS가 Claude CLI OAuth 토큰을 사용하므로 Models API용 `sk-ant-api...` 키가 없으면 실패가 아니라 `skipped: missing_anthropic_api_key`로 기록한다.
- GPT-5.5 반영
  - Codex CLI catalog 기준 `gpt-5.5`를 Codex provider 기본/선택 모델에 추가했다.
  - OpenAI API catalog에서도 `gpt-5.5`가 발견될 수 있으므로 OpenAI `gpt-5.5`와 Codex `gpt-5.5`는 provider를 기준으로 별도 row로 유지한다.
  - 표시명은 `openai:gpt-5.5 => GPT-5.5`, `codex:gpt-5.5 => GPT-5.5 (Codex CLI)`로 분리한다.
- 채팅 모델 선호도
  - `chat_model_preferences`의 PK는 기존 `model_id`에서 `preference_key`로 전환했다.
  - `preference_key` 형식은 `provider:model_id`이며, 자동 라우팅만 예외적으로 `mixture`를 사용한다.
  - 상단고정/즐겨찾기/숨김/노출순서는 `codex:gpt-5.5`와 `openai:gpt-5.5`에 독립 적용된다.
- 운영 API
  - `GET /api/v1/llm-models/discovery-runs`
  - `GET /api/v1/llm-models/chat-preferences`
  - `PUT /api/v1/llm-models/chat-preferences`
- 검증 기준
  - 인증 API에서 `codex:gpt-5.5:true`, `openai:gpt-5.5:false`처럼 provider별 pin 상태가 분리되어야 한다.
  - 라우팅은 `openai:gpt-5.5 -> openai_compatible_direct`, `codex:gpt-5.5 -> codex_cli`로 분기되어야 한다.
...
| 날짜 | 버전 | 변경 |
|------|------|------|
| 2026-04-28 | v5.0 | 공급자 catalog 자동 발견, `llm_model_discovery_runs`, Codex/OpenAI GPT-5.5 분리, provider-scoped 채팅 모델 선호도 도입 |
