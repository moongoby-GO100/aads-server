# spec: obys-v4-audit-detail

## 의도(무엇/왜)
"계정 권한 / 완료 / 전환 이력" 화면. 오비서 사이드바에서 통합 승인함, 권한·감사로그, 사업자·지점 메뉴가 이 화면을 참조한다.

## 범위경계
- 소유: 통합 승인함 / 권한·감사로그 / 사업자·지점
- 상태: 미정 — 정본 소유 메뉴 없이 여러 메뉴가 참조, 점검 필요
- ⚠️ 충돌: 통합 승인함, 권한·감사로그, 사업자·지점 가 동시 참조한다. 정본 소유 메뉴를 CEO/PM이 확정해야 한다.
