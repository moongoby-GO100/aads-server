...
## 2026-05-05 상품등록 UI 기획 v1.3 기록

- CEO 지시: 뉴톡V1 상세이미지 등록 프로세스(`/products/goods_img/{GoodsCode}` → `/products/goods_img_sorting_test1/{GoodsCode}` → 상세 HTML 생성 → 단하루상품설명/소매 엑셀 제공)를 확인하고 V2 통합관리 방안을 기획.
- 확인 근거:
  - V1 원본 이미지 등록: `/home/danharoo/www/application/controllers/products.php::goods_img`, `goods_img_upload`
  - V1 원본 이미지 화면: `/home/danharoo/www/application/views/products/goods_img.php`
  - V1 상세 이미지 정렬: `/home/danharoo/www/application/controllers/products.php::goods_img_sorting_test1`
  - V1 상세 정렬 화면: `/home/danharoo/www/application/views/products/goods_img_sorting_test1.php`
  - V1 상세 HTML 생성/저장: `/home/danharoo/www/minimall/controllers/goods.php::detail_save`, `images_save_end`
  - V1 소매 엑셀 제공: `/home/danharoo/www/application/controllers/products.php::goods_select_newtalk_make`, `goods_excel_newtalk_data`
  - V1 운영 데이터: `bag513k64` 원본/썸네일 포함 파일 152개, `GoodsDetailSave=Y`, `DeSkin=8`, `MoSkin=1`, `DanharooDescription` 2,226자, `GoodsSortImg1/2/3/4` 저장값 존재.
- 산출물:
  - 로컬 원본: `NT-PRODUCT-REGISTRATION-UI-PLAN-v1.3-20260505.html`
  - 임시 기획 폴더: `tmp_ntv2_plans/NT-PRODUCT-REGISTRATION-UI-PLAN-v1.3-20260505.html`
- 핵심 결정 초안:
  - 상세이미지 등록은 V2 상품등록 필수 단계로 포함한다.
  - V2 상품등록 화면에 `이미지 작업실`과 `상세페이지 빌더` 탭을 추가한다.
  - 원본 이미지 저장소와 판매 상세 산출물은 분리 관리한다.
  - V1의 `GoodsSortImg1~4`, `DeSkin`, `MoSkin`, `DanharooDescription`, 생성 산출물, 엑셀 제공 상태를 V2 데이터 모델에 반영한다.
  - 초기 P0는 V1 이미지/상세 HTML 읽기 전용 미러링 API와 V2 관리자 조회 화면으로 시작한다.
- CEO 확인 필요:
  - V2 초기 스킨 정책을 V1 PC/MO 스킨 1~8 유지로 갈지, 대표 스킨 1개로 통합할지.
  - 상세 HTML 생성 방식을 V1처럼 이미지 합성 파일 중심으로 둘지, V2 HTML/CSS 렌더링 원본 중심으로 둘지.
  - 소매 엑셀 제공 권한을 구독 등급별 제한으로 둘지, V1처럼 다운로드 로그 중심으로 둘지.
